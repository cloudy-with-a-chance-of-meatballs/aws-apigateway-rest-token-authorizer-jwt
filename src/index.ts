...
export class AwsApigatewayRestTokenAuthorizerJwt {
  public async getAuthResponse(
    event: APIGatewayTokenAuthorizerEvent,
    o: ITokenAuthorizerOptions,
    ...
    switch (o.verificationStrategy.strategyName) {
      case 'jwksFromUriByKid':
        secret = (await this.getPublicKeyFromJwksUri(
          o.verificationStrategy.uri,
          o.verificationStrategy.kid,
        )) as Secret;
    }
    try {
      let decodedJwt: any = jwt.verify(
        event.authorizationToken,
        secret as Secret,
      ) as any;
      switch (o.payloadValidationStrategy?.strategyName || '') {
        case 'schema':
          decodedJwt = this.validateJwtPayloadAgainstJsonSchema(
            decodedJwt,
            o.payloadValidationStrategy!.schema,
          );
      }
      return this.authResponseAllow(decodedJwt);
    } catch (e) {
      return this.authResponseError(
        (e as Error).name.toUpperCase(),
        (e as Error).message,
      );
    }
  }
  ...
  private authResponseError(type: string, message: string): AuthResponse {
    const policy = this.authorizerPolicyDocument(
  ...
  private async getPublicKeyFromJwksUri(
    jwksUri: string,
    jwksKid: string,
  ): Promise<Secret> {
    try {
      ...
      throw new Error(
        AuthorizerErrorConstants.JwksPrefix + (e as Error).name.toUpperCase(),
      );
    }
  }

  private validateJwtPayloadAgainstJsonSchema(
    payload: any,
    schemaAsJson: string,
  ): any {
  ...
  private authorizerPolicyDocument(
    effect: PolicyStatementEffect,
  ...
  private flatten(
    ...
    if (Array.isArray(input)) {
      input.forEach((value, index) => {
        this.flatten(value, `${prefix}${prefix ? '.' : ''}${index}`, result);
      });
    } else if (input && typeof input === 'object') {
      Object.keys(input).forEach((key) => {
        this.flatten(input[key], `${prefix}${prefix ? '.' : ''}${key}`, result);
      });
    } else {
      result[prefix] = input;
    }
